...
import { ChevronRight, Sparkles, BookOpen, Users, Award, Library, Heart, UserRound, X } from "lucide-react"
...
export default function SchoolActivities() {
  ...
  return (
    <section className="py-16 bg-[#f0e6d2] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 pt-8 relative z-10" ref={containerRef}>
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-[#8b1a1a] mb-4 font-serif">Our Activities</h2>
          <p className="text-[#5a3e36] max-w-2xl mx-auto">OUR BEST SERVICES FOR YOUR KIDS</p>
        </div>
            ...
            >
              ...
              <div className="p-5">
                <div className="flex items-center mb-3">
                  ...
                  <h3 className="text-lg font-bold text-[#8b1a1a]">{activity.title}</h3>
                </div>
                <p className="text-sm text-[#5a3e36] line-clamp-3">{activity.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
                ...
                {activities.find((a) => a.id === activeActivity) && (
                  <>
                    <div className="relative h-64">
                      <Image
                        src={activities.find((a) => a.id === activeActivity)?.image || ""}
                        alt={activities.find((a) => a.id === activeActivity)?.title || ""}
                        fill
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                        <div className="flex items-center">
                          <div className="bg-white/20 p-2 rounded-full mr-3">
                            <div className="text-white">{activities.find((a) => a.id === activeActivity)?.icon}</div>
                          </div>
                    ...
                    <div className="p-6">
                      <p className="text-[#5a3e36] mb-4 leading-relaxed">
                      ...
                      <div className="mt-6 flex justify-between items-center">
                        <div className="flex items-center text-[#8b1a1a]">
                          ...
                          <span className="font-medium">Enriching young minds</span>
                        </div>
                        <button
                          className="px-4 py-2 bg-[#8b1a1a] text-white rounded-md hover:bg-[#8b1a1a]/90 transition-colors"
                          onClick={() => setActiveActivity(null)}
                        >
                          Close
                        </button>
