...
import { Bell, ChevronRight, ChevronLeft, MoreHorizontal } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/50 backdrop-blur-sm">
      <motion.div
        ...
        animate={{ scale: 1, y: 0 }}
        exit={{ scale: 0.9, y: 20 }}
        className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] sm:max-h-[80vh] overflow-hidden mx-2 sm:mx-0"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="bg-gradient-to-r from-[#8b1a1a] to-[#a52a2a] text-white p-4 sm:p-6 flex justify-between items-center">
          <div className="flex items-center">
            <Bell className="h-5 w-5 sm:h-6 sm:w-6 mr-2 sm:mr-3" />
            <h2 className="text-lg sm:text-xl font-bold">All Announcements</h2>
          </div>
          <button
            onClick={onClose}
            className="text-white hover:bg-white/20 p-1.5 sm:p-2 rounded-full transition-colors text-lg sm:text-xl"
          >
            ✕
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-120px)] sm:max-h-[60vh]">
          {isLoading ? (
            ...
            <div className="text-center text-red-600 py-8 text-sm sm:text-base">{error}</div>
            ...
            <div className="space-y-3 sm:space-y-4">
              {announcements.map((announcement, index) => (
                <div
                  ...
                  className="bg-[#f8f3e9] p-3 sm:p-4 rounded-lg border border-[#d4b483]/30 hover:shadow-md transition-shadow"
                >
                  <div className="flex items-start">
                    <div className="bg-[#8b1a1a]/10 p-1.5 sm:p-2 rounded-full mr-3 sm:mr-4 flex-shrink-0">
                      <Bell className="h-4 w-4 sm:h-5 sm:w-5 text-[#8b1a1a]" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base sm:text-lg font-medium text-[#8b1a1a] mb-1 leading-tight">
                        {announcement.title}
                      </h3>
                      {announcement.date && (
                        <div className="flex items-center text-xs sm:text-sm text-[#5a3e36] mb-2">
                      ...
                      <p className="text-xs sm:text-sm text-[#5a3e36] opacity-70">
                        Posted on{" "}
...
export default function AnnouncementsTicker() {
  ...
  return (
    <>
      <div 
        className="bg-gradient-to-r from-[#8b1a1a]/10 via-[#8b1a1a]/5 to-[#8b1a1a]/10 border-2 border-[#d4b483]/40 rounded-xl p-3 sm:p-4 shadow-lg hover:shadow-xl transition-shadow duration-300 mx-2 sm:mx-0"
      ...
      >
        ...
        <div className="lg:hidden">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center bg-gradient-to-r from-[#8b1a1a] to-[#a52a2a] text-[#f8f3e9] px-3 py-2 rounded-lg shadow-md">
              <Bell className="h-4 w-4 sm:h-5 sm:w-5 mr-2 animate-pulse" />
              <span className="text-xs sm:text-sm font-bold tracking-wide">ANNOUNCEMENTS</span>
            </div>
            
            <button
              onClick={handleAnnouncementClick}
              className="bg-[#8b1a1a] hover:bg-[#a52a2a] text-white px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors duration-200 shadow-md hover:shadow-lg"
            >
              View All
            </button>
          </div>
          ...
          {/* Announcement Display */}
          <div className="overflow-hidden relative bg-white/50 rounded-lg border border-[#d4b483]/20 mb-3" style={{ minHeight: '60px' }}>
            {isLoading ? (
              <div className="absolute inset-0 flex items-center px-3 sm:px-4">
                <div className="animate-pulse bg-gray-300 h-4 sm:h-5 w-4/5 rounded"></div>
              </div>
            ) : (
              <div className="relative h-full w-full overflow-hidden">
                <motion.div
                  ...
                  className="absolute inset-0 flex items-center px-3 sm:px-4 cursor-pointer hover:bg-[#d4b483]/10 transition-colors duration-200"
                  onClick={handleAnnouncementClick}
                >
                  <p className="text-sm sm:text-base font-medium text-[#5a3e36] hover:text-[#8b1a1a] transition-colors duration-200 leading-relaxed py-2">
                    {announcements[currentIndex]}
                  </p>
                </motion.div>
              </div>
                ...
                {autoplay && !isPaused ? (
                  <div className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center">
                    <div className="flex space-x-0.5 sm:space-x-1">
                      <div className="w-0.5 sm:w-1 h-2 sm:h-3 bg-current"></div>
                      <div className="w-0.5 sm:w-1 h-2 sm:h-3 bg-current"></div>
                    </div>
                  </div>
                ) : (
                  <div className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center">
                    <div className="w-0 h-0 border-l-[5px] sm:border-l-[6px] border-l-current border-t-[3px] sm:border-t-[4px] border-t-transparent border-b-[3px] sm:border-b-[4px] border-b-transparent ml-0.5 sm:ml-1"></div>
                  </div>
                )}
              </button>
        ...
        <div className="hidden lg:flex justify-center mt-3 space-x-1">
        ...
        <div className="hidden lg:flex justify-center mt-2 items-center space-x-2">
        ...
        <div className="lg:hidden flex justify-center mt-3 space-x-1">
          {announcements.map((_, index) => (
            <button
              key={index}
              onClick={() => {
